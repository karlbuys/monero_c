...
import 'package:cw_lws/monero.dart' as monero;
...
class MoneroWallet implements Wallet2Wallet {
  ...
  String exchangeMultisigKeys(
      {required List<String> info, required bool force_update_use_with_caution}) {
    return monero.Wallet_exchangeMultisigKeys(walletPtr,
        info: info, force_update_use_with_caution: force_update_use_with_caution);
  }
  ...
  void init3(
    ...
    return monero.Wallet_init3(walletPtr,
        argv0: argv0, defaultLogBaseName: defaultLogBaseName, logPath: logPath, console: console);
  }
  ...
  bool verifySignedMessage(
      {required String message, required String address, required String signature}) {
...
class MoneroWalletManager implements Wallet2WalletManager {
  ...
  Wallet2Wallet createDeterministicWalletFromSpendKey(
      ...
      int kdfRounds = 1}) {
    final wallet = monero.WalletManager_createDeterministicWalletFromSpendKey(wmPtr,
        path: path,
        password: password,
        language: language,
  ...
  Wallet2Wallet createWallet(
    ...
    final wallet = monero.WalletManager_createWallet(wmPtr,
        path: path, password: password, language: language, networkType: networkType);
    return MoneroWallet(wallet);
  }
  ...
  int queryWalletDevice(
      {required String keysFileName, required String password, required int kdfRounds}) {
